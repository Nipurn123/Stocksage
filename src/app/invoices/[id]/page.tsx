...
const SAMPLE_INVOICE: Invoice = {
  id: 'sample-invoice',
  invoiceNumber: 'INV-SAMPLE-123456',
  date: new Date().toISOString(),
  dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
  vendorName: 'DigitalEdge Solutions',
  vendorAddress: '123 Tech Plaza, Suite 400, San Francisco, CA 94105',
  vendorEmail: 'billing@digitaledge.com',
  vendorPhone: '(415) 555-1234',
  customerName: 'StockSage Inc.',
  customerAddress: '456 Business Plaza, Suite 200, San Francisco, CA 94107',
  totalAmount: 540.98,
  taxAmount: 42.49,
  status: 'pending',
  ...
  notes: "Thank you for your business! Payment is due within 30 days. For questions regarding this invoice, please contact billing@digitaledge.com."
};
  ...
  useEffect(() => {
    const fetchInvoiceDetails = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          // Try to create a sample invoice if we couldn't find the requested one
          if (data.message === 'Invoice not found') {
            try {
              // Generate a different invoice type based on the invoiceId
              const invoiceTypes = ['standard', 'premium', 'maintenance', 'consulting', 'hardware'];
              const typeIndex = Math.abs(invoiceId.charCodeAt(0) + invoiceId.charCodeAt(invoiceId.length - 1)) % invoiceTypes.length;
              const invoiceType = invoiceTypes[typeIndex];
              
              // Create a sample invoice with the determined type
              const sampleResponse = await fetch(`/api/invoices/sample?type=${invoiceType}`);
              const sampleData = await sampleResponse.json();
              
              if (sampleResponse.ok && sampleData.success) {
                // Redirect to the newly created sample invoice
                router.push(`/invoices/${sampleData.invoiceId}`);
                return;
              } else {
                throw new Error(data.message || 'Failed to fetch invoice');
              }
            } catch (sampleErr) {
              console.error('Error creating sample invoice:', sampleErr);
              throw new Error(data.message || 'Failed to fetch invoice');
            }
          } else {
            throw new Error(data.message || 'Failed to fetch invoice');
        ...
        // For demo purposes, show a sample invoice if we can't fetch the real one
        if (process.env.NODE_ENV === 'development') {
          console.log('Using sample invoice as fallback');
          setInvoice(SAMPLE_INVOICE);
          toast.success('Displaying sample invoice data for demonstration');
        } else {
